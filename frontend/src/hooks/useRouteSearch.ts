...
export const useRouteSearch = (
  ...
  useEffect(() => {
    if (!fromPoint || !toPoint) {
      ...
      console.info(
        `${ROUTE_SEARCH_LOG_PREFIX} Search reset because one or both points are missing.`,
        { fromPoint, toPoint },
      )
    ...
    return () => {
      controller.abort()
      console.debug(`${ROUTE_SEARCH_LOG_PREFIX} Cleanup aborted in-flight request.`, {
        requestId,
      })
    }
  }, [fromPoint, toPoint, bufferRadius])

  return { results, status, error, emptyMessage }
}
